"""
    Recieves automated webhooks sent by Wix regarding invoices being overdue. this is a test file used for logging
"""
...
def invoice_overdue(request):
    try:
        """Recieves webhooks regarding an invoice being overdue

        ### Raises:
        """
        utilites.log("INFO", str(request.body)) #Logs request body

        logRow = TestingLogs(log = str(request.body), type = "OVERDUE")
        logRow.save()
